...
class Alpha():
    ...
    def adjacency_mat(self):
        adj = np.array(nx.adjacency_matrix(self.graph, weight=None).todense())
        return adj
    ...
    def edges_extracting(self, adj_sym, sym=True):
        # find all edges 
    ...
    def nonback(self, all_edg_sym, adj=None, flow=False):
        ...
        for i in range(np.shape(all_edg_sym)[0]): 
            for j in range(np.shape(all_edg_sym)[0]): 
                if all_edg_sym[i,1] == all_edg_sym[j,0] and all_edg_sym[i,0] != all_edg_sym[j,1]:
                    if flow and degrees[all_edg_sym[j][0]] > 1:
                      nonback[i,j] = 1/(degrees[all_edg_sym[j][0]] - 1)
                    else: 
                      nonback[i,j] = 1
                if i==j and not i%200:
                  print(i, end=' ')
        return nonback

    #Flowmatrix construction function (from adjacency matrix, nonbacktracking matrix and list of edges)
    def flowmat(self, adj, nonback, all_edg_sym, copy=False):
        degrees = np.sum(adj, axis=1)
        if copy:
    ...
    def translation_eig_vec(self, eig_vals, eig_vecs, edges, adj, max_clust_num, tail=False, test=False):
        degrees = np.sum(adj, axis=1)
        if test:
          degrees=degrees+np.sum(adj, axis=0)
